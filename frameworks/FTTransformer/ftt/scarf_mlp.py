...
class SCARF(nn.Module):
    def __init__(
        ...
        self.fusion = FusionTokenizer(cat_dims, n_con, emb_dim)
        n_input = (len(cat_dims) + n_con) * emb_dim

        if encoder:
            self.encoder = encoder
        else:
            self.encoder = MLP(n_input, emb_dim, encoder_depth)
        ...
        self.corruption_len = int(corruption_rate * n_input)
    ...
    def forward(self, anchor_cat, anchor_con):
        ...
        anchor = self.fusion(anchor_cat, anchor_con)

        positive_con = self.corruption(anchor_con)
        positive_cat = self.corruption(anchor_cat).type(torch.int64)
        positive = self.fusion(positive_cat, positive_con)
